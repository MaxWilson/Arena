...
let Tests() = (testLabel "Unit") <| testList "UI" [
    testCase "Spot-check AutoFight calibrate using mocked evaluation" <| fun () ->
        ...
        test <@ (findRange eval None |> Async.RunSynchronously) = (11, 30) @>
        test <@ (findRange eval (Some 20) |> Async.RunSynchronously) = (11, 20) @>
        test <@ (findRange eval (Some 5) |> Async.RunSynchronously) = (5, 5) @>
    ]
